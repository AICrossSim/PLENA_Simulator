...
def decoder_layer_flops(seq_len: int, hidden: int, inter: int, num_heads: int = 1) -> int:
    ...
    ffn         = (2 * seq_len * hidden * inter +   # gate
                   2 * seq_len * hidden * inter +   # up
                   2 * seq_len * inter  * hidden)   # down
...
def roofline_analysis(
    ...
    print(f"    Achieved BW        : {achieved_bw_gbs*1e3:.1f} MB/s  ({achieved_bw_gbs/peak_bw*100:.2f}% of peak)")
    print(f"    Achieved compute   : {achieved_gflops*1e3:.2f} MFLOPS  ({achieved_gflops/peak_compute*100:.4f}% of peak)")
    print(f"\n  Roofline:")
...
def main():
    ...
    parser.add_argument("--config",      choices=["behavior", "analytic"], default="behavior",
                        help="Hardware config section from plena_settings.toml")
    args = parser.parse_args()

    if args.latency_ns and args.hbm_bytes:
        roofline_analysis(args.latency_ns, args.hbm_bytes,
                          args.seq_len, args.hidden, args.inter, args.config)
    else:
        ...
        if parsed is None:
            ...
            print("Usage: bash run.sh build smollm2_135m_decoder 2>&1 | python3 analytic_models/roofline/decoder_roofline.py")
            sys.exit(1)
        roofline_analysis(parsed["latency_ns"], parsed["hbm_bytes"],
                          args.seq_len, args.hidden, args.inter, args.config)
