...
def _design(rows: list[dict]) -> tuple[np.ndarray, np.ndarray]:
    X = np.array(
        [[feature_row(d["MLEN"], d["BLEN"], d["t"], d["l"], d["s"])[n] for n in FEATURE_NAMES] for d in rows],
        float,
    )
    ...
    while np.any((~passive) & (gradient > tolerance)):
        ...
        while np.any(passive & (trial <= tolerance)):
            ...
            if not np.any(can_step):
                ...
                alpha = np.min(coefficients[infeasible][can_step] / denominator[can_step])
...
def fit_mode(mode: str) -> dict:
    ...
    anchor_ho = _predict(holdout_coefficients, 1024, 4, 4, 4) if mode == "mxint" else None
...
def _block_design(
    ...
    design = np.array([[row_function(row)[name] for name in names] for row in rows], dtype=float)
...
def fit_full_chip(matrix_fits: dict[str, dict]) -> dict[str, dict]:
    ...
    for row in holdout:
        ...
        for block, (names, row_function, _) in _FULL_CHIP_BLOCKS.items():
            ...
            predicted += sum(holdout_coefficients[block][index] * features[name] for index, name in enumerate(names))
