...
def fp_issue_pipeline_counts(config: Mapping[str, Any]) -> dict[str, int]:
    ...
    register_address_bits = int(config.get("FP_OPERAND_WIDTH", RTL_FP_OPERAND_WIDTH))
    if depth < 2:
...
def loop_address_generator_counts(config: Mapping[str, Any]) -> dict[str, int]:
    ...
    register_index_width = int(config.get("INT_OPERAND_WIDTH", RTL_INT_OPERAND_WIDTH))
    ...
    address_streams = int(config.get("LOOP_ADDRESS_STREAMS", RTL_LOOP_ADDRESS_STREAMS))
    if (
        min(
            address_width,
    ...
    frame_counter_bits = loop_depth * (loop_count_width + register_index_width)
    ...
    status_bits = 2
    sequential_bits = (
        shadow_value_bits + shadow_valid_bits + pending_state_bits + frame_state_bits + stack_pointer_bits + status_bits
    )
    ...
    shadow_read_mux_equivalent_bits = 3 * (register_count - 1) * address_width
    pending_comparator_bits = address_streams * register_index_width
    datapath_equivalent_bits = address_adder_bits + counter_decrement_bits + shadow_read_mux_equivalent_bits
    ...
    return {
        ...
        "storage_equivalent_bits": storage_state_bits + datapath_equivalent_bits,
...
def feature_row(config: Mapping[str, Any]) -> dict[str, float]:
    ...
    return {
        "fp_vector_buffer_bits": float(counts["fp_vector_buffer_elements"] * _fp_width(config)),
...
def estimate_scalar_area(
    ...
    *,
    ...
    calibrated_dc_area = sum(float(coefficients.get(name, 0.0)) * features[name] for name in FEATURE_NAMES)
    ...
    issue_structural_dc_area = (
        storage_equivalent_bits * storage_coefficient + control_equivalent_bits * control_coefficient
    )
    ...
    loop_structural_dc_area = (
        loop_storage_equivalent_bits * storage_coefficient + loop_control_equivalent_bits * control_coefficient
    )
    loop_structural_area = loop_structural_dc_area * scale
    calibrated_area = calibrated_dc_area * scale
    selected_structural_dc_area = (issue_structural_dc_area if issue_enabled else 0.0) + (
        loop_structural_dc_area if loop_enabled else 0.0
    )
    selected_structural_area = (issue_structural_area if issue_enabled else 0.0) + (
    ...
    extrapolated = (
        ...
        or str(config.get("FP_SETTING", "FP_E5M6")).upper() not in CALIBRATION_DOMAIN["FP_SETTING"]
    )
