...
A separate Design Compiler run synthesised ``matrix_machine`` on its own -
...
two vendored measurement tables and returns one record:
...
``leakage_density``
    leakage per unit area, which the synthesis reports give directly and which needs
...
``compute_energy_envelope``
    ...
    a reference ``0.203 pJ/MAC`` compute-energy anchor.

**Scope.** Every number here is ``matrix_machine`` alone, in um^2, at MLEN
16-64, at 25 C. Full-chip estimates from this package are mm^2 at MLEN
...
#: Reference MAC energy anchor used by consumers of this model, cross-checked
#: against the measured envelope but not refitted.
ANALYTIC_PJ_PER_MAC = 0.203

#: Independent leakage-density fit quoted with the measurements, retained only as an
...
SCOPE: dict[str, Any] = {
    ...
    "families": ["mxfp"],
    "not_comparable_to": (
        "full-chip mm^2 estimates at MLEN 128-1024; this run measures one block, in um^2, over MLEN 16-64, at 25 C"
    ),
    "timing_closed": True,
}
...
def _least_squares(matrix: Sequence[Sequence[float]], target: Sequence[float]):
    ...
    for pivot in range(columns):
        ...
        for row_index in range(columns):
            ...
            normal[row_index] = [
                value - factor * pivot_value for value, pivot_value in zip(normal[row_index], normal[pivot])
            ]
...
def precision_law(points: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    ladder = [point for point in points if point["mlen"] == LADDER_MLEN and point["blen"] == LADDER_BLEN]
    if len(ladder) < 3:
    ...
    constant, per_exp, per_mant = _least_squares(design, [p["area_um2"] for p in ladder])
    residuals = {
        p["precision"]: (constant + per_exp * p["exp_bits"] + per_mant * p["mant_bits"]) / p["area_um2"] - 1.0
        for p in ladder
    }
    ...
    return {
        ...
        "residual_pct_by_format": {name: value * 100.0 for name, value in sorted(residuals.items())},
...
def geometry_law(points: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    per_pe = {f"{p['precision']}_{p['mlen']}x{p['blen']}": p["area_um2"] / p["pes"] for p in points}
    ...
    if holdout:
        ...
        if anchors:
            ...
            check = {
                ...
                "error_pct": (predicted - retained["area_um2"]) / retained["area_um2"] * 100.0,
                "note": (
                    "constant area per PE, extrapolated from the smallest measured shape to the retained largest one"
                ),
            }
...
def leakage_density(points: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    """Derive leakage per unit area, which the synthesis reports measure directly."""

    densities = [p["leakage_uw"] * 1e-3 / p["area_um2"] for p in points]
    mean = statistics.fmean(densities)
    ladder = [
        p["leakage_uw"] * 1e-3 / p["area_um2"] for p in points if p["mlen"] == LADDER_MLEN and p["blen"] == LADDER_BLEN
    ]
    return {
        "mw_per_um2": mean,
        "w_per_mm2": mean * 1e3,
        ...
        * 100.0,
        ...
        "caveat": (
            ...
            "hot-corner point was synthesised in this run"
        ),
    }
...
def census_cross_validation(
    ...
    for point in points:
        ...
        predicted = float(estimate_matrix_machine_area(config, corner="dc")["area"])
        comparisons.append(
            {
    ...
    for entry in comparisons:
        entry["error_after_uniform_offset_pct"] = (entry["ratio"] / offset - 1.0) * 100.0
    residuals = [abs(entry["error_after_uniform_offset_pct"]) for entry in comparisons]
    return {
        "points": comparisons,
        "uniform_offset_census_over_measured": offset,
        "raw_error_pct": {
        ...
        "verdict": (
            ...
            "Design Compiler runs, not a shape or precision error"
        ),
        "known_model_limit": (
            "the census features depend on total operand width only, so it "
            "cannot separate two MXFP formats of equal width; the measurements "
            "place E1M2 and E2M1 a few percent apart and that difference "
...
def compute_energy_envelope(
    ...
    *,
    ...
    """Bracket the reference compute-energy anchor inside the declared-activity sweep."""
    ...
    for point_id in sorted({row["point_id"] for row in rows}):
        ...
        for low, high in pairwise(sweep):
            if low["pj_per_mac"] <= anchor_pj_per_mac <= high["pj_per_mac"]:
                ...
                implied[key] = low["toggle_rate"] + fraction * (high["toggle_rate"] - low["toggle_rate"])
                break
    if not implied:
        raise ValueError("no measured sweep brackets the reference anchor")
    ...
    return {
        ...
        "verdict": (
            "the reference anchor sits inside the measured envelope at a toggle "
        ...
        "evidence_scope": (
            ...
            "and propagated by the synthesis tool, not measured from workload "
...
def build_record() -> dict[str, Any]:
    """Derive the complete gate-level validation record from the measured tables."""
    ...
    return {
        ...
        "decision": (
            ...
            "synthesis runs, and refitting on eight small-geometry MXFP "
...
def write_record(path: Path = ARTIFACT) -> dict[str, Any]:
    """Refresh the stored artifact from the vendored measurement tables."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        "  census offset "
        f"{census['uniform_offset_census_over_measured']:.4f}x, "
        "residual after offset "
        f"{census['shape_and_precision_error_after_offset_pct']['max']:.2f}% max"
    )
    print(f"  leakage {record['leakage_density']['w_per_mm2']:.6f} W/mm^2 at 25 C (matrix machine only)")
    return 0
