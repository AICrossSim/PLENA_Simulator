...
def structural_counts(config: Mapping[str, Any]) -> dict[str, int]:
    ...
    if sides["mode"] == "mxint":
        narrow_adapter_bits = mlen * ((8 - key_value.element_width) + (8 - weight.element_width)) + vlen * (
            (8 - activation.element_width) + (8 - key_value.element_width) + (8 - weight.element_width)
        )
    else:
...
def feature_row(config: Mapping[str, Any]) -> dict[str, float]:
    ...
    return {
        **{name: float(counts[name]) for name in FEATURE_NAMES if name != "fixed_control"},
        "fixed_control": 1.0,
    }
...
def _uniform_precision(config: Mapping[str, Any]) -> bool:
    values = {
        ...
        str(config.get("WEIGHT_WIDTH", "MXINT4")).upper().replace("MXINT_", "MXINT"),
    }
    return len(values) == 1 and next(iter(values)) in CALIBRATION_DOMAIN["uniform_precision"]
...
def estimate_hbm_interface_area(
    ...
    *,
    ...
    dc_area = sum(float(coefficients.get(name, 0.0)) * features[name] for name in FEATURE_NAMES)
