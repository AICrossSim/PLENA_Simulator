...
def segment_parallel_reduction_counts(config: Mapping[str, Any]) -> dict[str, int]:
    ...
    if segments < 1 or segments > vlen or vlen % segments:
        raise ValueError("REDUCTION_SEGMENTS must be a positive divisor of VLEN no larger than VLEN")
    if segments > 1 and segments != vlen // hlen:
        raise ValueError("segmented reduction requires REDUCTION_SEGMENTS == VLEN / HLEN")

    enabled = segments > 1
    fp_width = _fp_width(config)
    segment_width = vlen // segments
    tree_levels = math.ceil(math.log2(segment_width + 1))
    fifo_packets = RTL_SEGMENT_FIFO_MARGIN + tree_levels * RTL_REDUCTION_LEVEL_LATENCY if enabled else 0
    ...
    fifo_control_bits = 2 * fifo_pointer_width + 2 * fifo_count_width if enabled else 0
    ...
    destination_adder_bits = int(config.get("FP_OPERAND_WIDTH", RTL_FP_OPERAND_WIDTH)) if enabled else 0
    if enabled and destination_adder_bits <= 0:
        raise ValueError("FP_OPERAND_WIDTH must be positive")
    equivalent_bits = (
        fifo_data_bits + fifo_control_bits + drain_mux_equivalent_bits + drain_index_bits + destination_adder_bits
    )
    return {
        "segments": segments,
...
def feature_row(config: Mapping[str, Any]) -> dict[str, float]:
    ...
    fp_width = _fp_width(config)
    return {
        "element_and_reduction_bits": float((counts["lanes"] + counts["reduction_edges"]) * fp_width),
...
def estimate_vector_area(
    ...
    *,
    ...
    structural_dc_area = segment_counts["estimated_equivalent_bits"] * proxy_coefficient
    ...
    return {
        ...
        "segment_parallel_reduction_area_accounting": {
            ...
            "reason": ("no retained DC points contain the opt-in segmented-output FIFO and drain path"),
        },
        "evidence": evidence,
    }
