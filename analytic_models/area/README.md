...
stops at MLEN = 64 while design-space exploration reaches MLEN = 4096;
...
**This matters for how the results are read.** Reference-chip geometries
(MLEN = 1024 and above) are all outside the calibrated grid, so every full-chip
area at those sizes carries the extrapolation tier. The holdout errors below are
...
A later Design Compiler run synthesised `matrix_machine` on its own and was
...
**µm²**, over **MLEN 16–64**, at **25 °C**. Full-chip estimates from this package
are in **mm²**, at **MLEN 128–1024** and above. These are not the same
quantity and this run is not an error bar on those estimates; the artifact
carries the boundary in its `scope` record so it travels with any consumer.

What the gate-level run establishes on its own:
...
- The offset is *not* a model defect. At the six geometries the two runs
  share, the shipped calibration CSV is itself **1.113×** the new measurements
  (1.098–1.126). The disagreement is a level difference between two synthesis
  runs — different setup, corner and RTL revision — and the census already
...
`MXFP_E2M1` — both 4 bits wide — are predicted identically. The gate-level run
...
and would not improve a single relative trade-off — the measurements confirm those
to within 3%. The right upgrade is a synthesis run that re-measures the *full chip*
at *reference-chip geometries*, not a refit onto a smaller block.

The same run priced two mapped netlists over a declared toggle envelope of
0.05–0.50, giving 0.113–1.126 pJ/MAC. A reference MAC energy anchor of
0.203 pJ/MAC (`ANALYTIC_PJ_PER_MAC`) falls inside that envelope at an implied
toggle of 0.0797 (32×4) and 0.0835 (16×4), and 0.073–0.093 across all six swept
geometries. That is an independent bracket on a compute-energy coefficient a
power model may adopt — but it is a **declared-activity estimate**: the toggle
rate is assumed and propagated by the synthesis tool, not measured from workload
switching. It brackets the anchor; it does not calibrate it. The record is kept
here, alongside the area cross-validation it was measured with, so that an energy
consumer can cite it with its scope intact.
...
a design-space search rather than a free configuration choice: when HBM channel
count is a searched axis, each additional interface unit is charged 0.6875 mm² of
...
The package has no dependency on the rest of `analytic_models`; it reads a plain
mapping of hardware-configuration keys (`MLEN`, `BLEN`, `VLEN`, `HLEN`,
`ACT_WIDTH`, `WEIGHT_WIDTH`, `KV_WIDTH`, SRAM sizing, and the opt-in structure
flags) and returns dictionaries. Two intended uses:

- design-space search — `estimate_system_area` prices a candidate configuration
  and `solve_geometry_for_area` inverts it against an area budget;
- power modelling — a leakage-per-area model needs the per-block areas in the
  `breakdown` dictionary, and the gate-level record above supplies a measured
  25 °C leakage density and a bracketed MAC-energy anchor for `matrix_machine`.

Any consumer should propagate `evidence_tier` alongside the number it reports;
`weakest_tier` is exported for that purpose.
