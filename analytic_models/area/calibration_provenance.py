...
_CALIBRATION_FILES = {
    "full_chip_anchors.csv": {
        "sha256": ("8a388d40f9567900652ca9991e50385ec2cf4bd76cfa9fef9ce48986ffd1d884"),
        "rows": 17,
    },
    "matrix_machine_mxint.csv": {
        "sha256": ("ace5b1e8fe4b778eb624f094ef028367a33ceb148a61fae8cfae378947fb8f40"),
        "rows": 48,
    },
    "matrix_machine_mxfp.csv": {
        "sha256": ("2c951a7a200fb6d95d957bd2d7ce11acfd808658972e9507050e853f6a01f1fd"),
        "rows": 23,
    },
    "asap7_sram_macro_table.csv": {
        "sha256": ("45c7e031b2c32efd5083379fdcdcd32361af39dbc930d474756ec56ea111bf07"),
        "rows": 36,
    },
    "matrix_structural_coefficients.json": {
        "sha256": ("bfe51635c498088d40d905f3cec62fbd5ed916863ed4911d8a6530c61eea2b41"),
    },
    "ASAP7_SRAM_LICENSE": {
        "sha256": ("f729eadf714ca54a7b665f9094f9687ba7b7f2c30e6c294431d1f0e2b1144245"),
    },
}
...
def _aggregate_gates(coefficients: Mapping[str, Any]) -> dict[str, Any]:
    ...
    holdout = all(
        float(report["holdout_fraction"]) >= 0.25 and float(report["p95_abs_error_pct"]) <= 10.0
    ...
    areas = tuple(estimate_matrix_machine_area(_config(mlen, blen))["area"] for mlen, blen in shapes)
    monotonic = all(right > left for left, right in pairwise(areas))

    precision_areas = tuple(
        estimate_matrix_machine_area(_config(1024, 4, token, token, "MXINT4"))["area"]
        for token in ("MXINT2", "MXINT4", "MXINT8")
    )
    precision = precision_areas[0] < precision_areas[1] < precision_areas[2]
    ...
    gates = {
        "holdout": {
            "passed": holdout,
            "minimum_fraction": min(float(report["holdout_fraction"]) for report in holdout_reports.values()),
            "maximum_p95_abs_error_pct": max(float(report["p95_abs_error_pct"]) for report in holdout_reports.values()),
            ...
            "per_block": {
                name: {
                    ...
                    "median_abs_error_pct": float(report["median_abs_error_pct"]),
    ...
    if tuple(gates) != _GATE_NAMES or not all(gate["passed"] for gate in gates.values()):
...
def build_area_calibration_audit(
    ...
    coefficients = json.loads((calibration / "matrix_structural_coefficients.json").read_text(encoding="utf-8"))
    resolved = tuple(root / path for path in sorted(raw_paths))
    existing = tuple(path for path in resolved if path.exists())
    if existing:
        raise ValueError("audit constants require absent raw DC paths; update the evidence grade")
...
def write_area_calibration_audit(
    ...
    if destination.exists():
        if destination.read_text(encoding="utf-8") != payload:
            raise FileExistsError(f"refusing to replace a different audit: {destination}")
        return destination
