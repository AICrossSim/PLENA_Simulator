...
large arrays (MLEN=1024, the reference chip), where a shape polynomial
...
def load_calibration_artifact(path: str | Path | None = None) -> dict[str, Any]:
    ...
    selected = Path(path or os.environ.get("PLENA_AREA_MATRIX_COEFFICIENTS") or DEFAULT_COEFFS).resolve()
...
def matrix_area_from_sides(mlen: int, blen: int, sides: Mapping[str, Any], coeffs: Mapping[str, float]) -> float:
...
def estimate_matrix_machine_area(
    ...
    if coeffs is None:
        raise FileNotFoundError(f"no fitted matrix coefficients for mode={mode}; run `python -m area.fit`")
    ...
    extrapolated = mlen not in calibration_domain["MLEN"] or blen not in calibration_domain["BLEN"]
    return {
        "area": dc_area * scale,
