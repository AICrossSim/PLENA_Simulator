...
The measurements these tests guard are evidence, not a fit input, and the honest
...
def test_measured_tables_are_present_and_complete() -> None:
    """The measured tables are vendored, so the record can always be re-derived."""
...
def _assert_equivalent(left, right, path: str = "") -> None:
    """Structural equality with a float tolerance for summation-order noise."""
    ...
    if isinstance(left, dict) and isinstance(right, dict):
        ...
        for index, (l_item, r_item) in enumerate(zip(left, right, strict=True)):
            _assert_equivalent(l_item, r_item, f"{path}[{index}]")
...
def test_stored_artifact_matches_its_derivation(stored: dict, derived: dict) -> None:
    ...
    _assert_equivalent(stored, derived)
...
def test_census_disagreement_is_a_uniform_offset_not_a_shape_error(
    ...
    If this ever fails the measurements have stopped confirming the census and the
    ...
    assert census["uniform_offset_census_over_measured"] == pytest.approx(1.1242, abs=0.005)
...
def test_equal_width_mxfp_formats_expose_a_declared_census_limit(
    ...
    by_key = {(entry["precision"], entry["geometry"]): entry for entry in stored["census_cross_validation"]["points"]}
    e1m2 = by_key[("MXFP_E1M2", "16x4")]
    e2m1 = by_key[("MXFP_E2M1", "16x4")]
    assert e1m2["census_dc_corner_um2"] == pytest.approx(e2m1["census_dc_corner_um2"], rel=1e-9)
...
def test_compute_energy_envelope_brackets_the_analytic_anchor(stored: dict) -> None:
    ...
    # The estimate is declared activity, not annotated workload switching, and
    # saying so is the whole basis on which it may be reported.
    assert "declared-activity" in envelope["evidence_scope"]
    assert "not measured from workload" in envelope["evidence_scope"]
...
def test_the_measurements_did_not_move_the_shipped_coefficients(stored: dict) -> None:
    """The census is validated by these measurements, not refitted onto them."""
    ...
    # gate-level tables cannot have become fit inputs by the back door.
