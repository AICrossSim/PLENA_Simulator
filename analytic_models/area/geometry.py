...
def solve_geometry_for_area(
    ...
    *,
    ...
    vlens = _positive_candidates("VLEN", vlen_candidates) if vlen_candidates is not None else mlens
    ...
    if not feasible:
        raise ValueError(f"no legal geometry fits {budget / 1e6:.6f} mm^2 ({legal} legal candidates evaluated)")
