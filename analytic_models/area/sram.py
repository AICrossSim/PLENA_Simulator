...
from .evidence import SRAM_MACRO_TABLE
...
@lru_cache(maxsize=8)
def _read_macros(path: str) -> tuple[dict[str, Any], ...]:
    p = Path(path)
    if not p.exists():
        return ()
    out = []
...
def _macros(path: str | Path | None = None) -> tuple[dict[str, Any], ...]:
    selected = Path(
        path or os.environ.get("PLENA_AREA_SRAM_MACRO_TABLE") or MACRO_TABLE
    ).resolve()
    return _read_macros(str(selected))
...
def _tile_area(
    depth: int, width: int, ports: int, macros: tuple[dict[str, Any], ...]
) -> float:
    ...
    return min(
        math.ceil(depth / m["depth"])
        * math.ceil(width / m["width"])
        * ports
        * m["area_um2"]
        for m in macros
    )
...
def estimate_sram_area(
    config: Mapping[str, Any], *, macro_table_path=None
) -> dict[str, Any]:
    ...
    m_depth = _depth(
        config, "MATRIX_SRAM_DEPTH", "MATRIX_SRAM_SIZE", default=max(32, 2 * mlen)
    )
    ...
    v_depth = _depth(
        ...
        "VECTOR_SRAM_DEPTH",
        "VECTOR_SRAM_SIZE",
        default=max(32, 256 + math.ceil(mlen / vlen)),
    )
    v_width = vlen * (
        _fp_width(config) + act.element_width + kv.element_width
    ) + 2 * scale_width * max(
        1, vlen // max(int(config.get("BLOCK_DIM", config.get("BLEN", 4))), 1)
    )
    # Scalar SRAMs (small, fixed): integer + FP register files, 1 port each.
    si = _tile_area(
        _depth(config, "INT_SRAM_DEPTH", default=32),
        int(config.get("INT_DATA_WIDTH", 32)),
        1,
        macros,
    )
    sf = _tile_area(
        _depth(config, "FP_SRAM_DEPTH", default=512), _fp_width(config), 1, macros
    )
    ...
    return {
        ...
        "evidence": {
            "tier": SRAM_MACRO_TABLE,
            "source": "calibration/asap7_sram_macro_table.csv",
            "model_scope": "minimum-area legal tiling of published single-port macros",
            "foundry_compiler_result": False,
        },
    }


def estimate_buffer_area(
    capacity_bytes: int,
    *,
    word_bits: int = 32,
    ports: int = 1,
    macro_table_path=None,
) -> dict[str, Any]:
    """Tile a small architecture buffer with the retained ASAP7 macros."""
    ...
    depth = math.ceil(capacity_bytes * 8 / word_bits)
    area = _tile_area(depth, word_bits, ports, _macros(macro_table_path))
    return {
        "area": area,
        "capacity_bytes": capacity_bytes,
        "depth": depth,
