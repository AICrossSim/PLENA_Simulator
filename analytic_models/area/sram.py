"""On-chip SRAM macro area for a PLENA die (um^2).
...
def _macros(path: str | Path | None = None) -> tuple[dict[str, Any], ...]:
    selected = Path(path or os.environ.get("PLENA_AREA_SRAM_MACRO_TABLE") or MACRO_TABLE).resolve()
...
def _tile_area(depth: int, width: int, ports: int, macros: tuple[dict[str, Any], ...]) -> float:
    ...
    return min(math.ceil(depth / m["depth"]) * math.ceil(width / m["width"]) * ports * m["area_um2"] for m in macros)
...
def estimate_sram_area(config: Mapping[str, Any], *, macro_table_path=None) -> dict[str, Any]:
    ...
    v_width = vlen * (_fp_width(config) + act.element_width + kv.element_width) + 2 * scale_width * max(
