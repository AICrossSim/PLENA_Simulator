...
print("=" * 75)
print(f"{'M×K×N':>14}  {'Tiles':>6}  {'Cyc/Tile':>8}  {'Sim Cycles':>10}  {'Sim Latency':>12}  {'RTL Cycles (fill in)':>20}")
print("-" * 75)
...
print(f"{'V_ADD_VV (behavioral)':20}  {'non-DC RTL':12}  {VECT_ADD_BEHAV:>10}  {6+3+VECT_ADD_BEHAV+1:>6}  {VECT_ADD_BEHAV:>6}  {rtl_measured:>12}  ← RTL measured")
print(f"{'V_ADD_VV (post-synth)':20}  {'DC_LIB RTL':12}  {VECT_ADD_SYNTH:>10}  {6+3+VECT_ADD_SYNTH+1:>6}  {VECT_ADD_SYNTH:>6}  {'[not tested]':>12}")
print(f"{'V_ADD_VV (simulator)':20}  {'dc_lib_en=1':12}  {VECT_ADD_SIM:>10}  {6+3+VECT_ADD_SIM+1:>6}  {VECT_ADD_SIM:>6}  {'[analytic]':>12}")
print()
print("VALIDATION RESULT:")
print(f"  RTL behavioral measured {rtl_measured} cycles == VECTOR_ADD_CYCLES={VECT_ADD_BEHAV} in configuration.svh → EXACT MATCH ✓")
...
print(f"  PE arithmetic pipeline depth (derived):  {RTL_SA_PE_PIPE_DEPTH} cycles  (= {RTL_SA_FIRST_TILE_CYCS} - {RTL_SA_BLOCK_DIM} input cycles)")
print(f"  Pipelined throughput (sim formula):       {SIM_THROUGHPUT_PER_TILE} cycles/tile  (= 1 + BLEN = 1 + {RTL_SA_BLOCK_DIM})")
...
print(f"    RTL latency = PE_pipeline_depth ({RTL_SA_PE_PIPE_DEPTH}) + input_cycles ({RTL_SA_BLOCK_DIM}) = {RTL_SA_FIRST_TILE_CYCS} ✓")
...
hidden = 64; heads = 2; kv_heads = 2; head_dim = 32
...
for seq_label, seq in cases:
    ...
    for label, m, k, n in ops:
        ...
        total_sim += sim_c; total_rtl += rtl_c
...
print(f"  V_RED_MAX       reduction_compute_tb     10 cyc (VLEN=8)    VECTOR_MAX_CYCLES=4             LATENCY>THPUT (expected) ✓")
print(f"  V_RED_SUM       reduction_compute_tb     14 cyc (VLEN=8)    VECTOR_SUM_CYCLES=20            VLEN=8 vs VLEN=64 model")
print(f"  S_EXP_FP        scalar_machine_tb        7 cyc (E5M10)      SCALAR_FP_EXP_CYCLES=2          RTL deeper than sim (dc_lib vs behavioral)")
print(f"  S_RECI_FP       scalar_machine_tb        4 cyc (E5M10)      SCALAR_FP_RECI_CYCLES=2         RTL deeper than sim (dc_lib vs behavioral)")
print(f"  MM_WO tile      matrix_machine_tb        35 cyc (1 tile)    1+BLOCK_DIM=9 (thput)           fill+drain overhead; amortizes in steady state")
print("=" * 85)
...
def table8():
    ...
    print("=" * W)
    print(f"  {'Instruction':<14}  {'RTL cocotb':>11}  {'Sim Constant':<26}  {'Sim Value':>9}  {'RTL/Sim':>7}  {'Match?'}")
    print("-" * W)
    ...
    rows = [
        ...
        ("V_RED_SUM", RTL_V_RED_SUM,  "VECTOR_SUM_CYCLES",      SIM_VECTOR_SUM_CYCLES,           "latency<throughput (VLEN=8 vs VLEN=64 model)"),
    ]
...
def table10():
    ...
    scalar_rows = [
        ("S_EXP_FP",  "fp_sfu", RTL_S_EXP_FP,  "SCALAR_FP_EXP_CYCLES",  SIM_SCALAR_EXP_CYCLES,
         "RTL pipeline deeper than sim model"),
        ("S_RECI_FP", "fp_sfu", RTL_S_RECI_FP, "SCALAR_FP_RECI_CYCLES", SIM_SCALAR_RECI_CYCLES,
         "RTL pipeline deeper than sim model"),
    ]
    ...
    print(f"  {'MM_WO Phase2 (drain)':<22}  {RTL_MM_TILE_PHASE2:>8}  {'':>16}  MCU drain (2×COMPUTE_DIM) + output pipeline")
    print(f"  {'MM_WO Total (1 tile)':<22}  {RTL_MM_TILE_TOTAL:>8}  {SIM_MM_TILE_THPUT:>16}  single (8,16)×(16,8) GEMM tile")
    print("=" * W)
