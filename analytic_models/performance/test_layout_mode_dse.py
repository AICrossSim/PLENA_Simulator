...
def test_dense_diagonal_balances_write_and_column_read_conflicts() -> None:
    cases = {case["layout"]: case for case in build_report()["dense_column_ablation"]["cases"]}
...
def test_real_mamba_and_kda_layouts_roundtrip_and_reach_read_ideal() -> None:
    ...
    assert report["nemotron3_mamba_decode"]["comparison"]["read_service_speedup"] == pytest.approx(53_176 / 14_904)
    ...
    assert report["kimi_k3_kda_decode"]["comparison"]["read_write_service_reduction_percent"] == pytest.approx(
        14.151465
    )
