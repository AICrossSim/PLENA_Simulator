...
transactional_emulator/testbench/
  ...
  tile_compiler.py   # Tile/memory manager (formerly sub_matrix_manager.py)
  model_layer_test_builder.py  # Shared HF model test infra
  *_aten_test.py           # Per-op tests
justfile / run.sh          # Build recipes
```

Call chain: `plena_program.py` → `developer_compiler.py` → `tile_compiler.py`
...
- `k_block_start, k_block_count` threaded through: `linear_ops.py` → `plena_program.py` → `developer_compiler.py` → `tile_compiler.py`.
