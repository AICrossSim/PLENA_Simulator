...
#: Power-of-two scale on the Qwen2-MoE gate weight. See the comment at its use.
...
def build_and_run(args: argparse.Namespace) -> dict:
    ...
    comparison_params = {
        ...
        "row_dim": mlen,
        # Exact: MXFP8-representable inputs mean quantization is the identity, and
        # the reference mirrors every BF16 rounding step the emitter performs.
        "atol": 0.0,
        "rtol": 0.0,
    }
...
def _assert_stage_attribution_healthy(build_dir: Path, *, gated: bool, combined: bool) -> None:
    ...
    drift by rewording, but they can be dropped entirely (silently falling back to
    substrings), can name a stage the emulator does not know, or can be emitted at
    the wrong boundary so a region bills to its neighbour.
    """
    profile = json.loads((build_dir / "stage_profile.json").read_text())
    coverage = profile["classification"]

    # Guard 1. Markers were actually used. If `moe_stage_marker` stopped being
    # emitted, the emulator would silently fall back to substring rules and
    # attribute this program's projections to the *routed* stages -- plausible
    # numbers, wrong branch.
