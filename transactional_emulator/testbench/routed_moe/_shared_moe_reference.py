...
operation**, so the comparison against the emulator can be exact rather than
tolerance-based. That means reproducing every intermediate BF16 rounding, not
just the mathematical result: ``silu(g) * u`` computed in float64 and rounded once
does not equal the hardware's six separately-rounded steps.
...
def shared_gate_golden(x: torch.Tensor, gate_weight_row: torch.Tensor) -> torch.Tensor:
    ...
    rounding. Wider shapes are compared with a tolerance instead.
...
def combine_shared_and_routed_golden(routed: torch.Tensor, shared: torch.Tensor) -> torch.Tensor:
    ...
    The shared branch is added unweighted in every architecture that has one; a
    route weight applies only to the routed experts.
    """
