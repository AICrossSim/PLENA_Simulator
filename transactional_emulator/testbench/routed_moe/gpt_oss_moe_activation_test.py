"""GPT-OSS MoE clamp-gated activation emulator smoke.

This is the third device-side step for fixed-routing MoE v0. It keeps the
expert projections split into gate/up, then executes the GPT-OSS non-standard
activation on PLENA vector ISA:

    gate = min(gate, 7)
    up   = max(min(up, 7), -7)
    sigmoid = 1 / (1 + exp(-1.702 * gate))
    hidden = (up + 1) * gate * sigmoid

Down projection, routing, and combine remain out of scope.
"""
...
def run_activation_smoke(args: argparse.Namespace) -> dict:
    mlen = args.mlen
    blen = args.blen
    ...
    print("=" * 80)
    print(
        "GPT-OSS MoE activation emulator smoke "
    ...
    golden = _activation_golden(gate, up)
    ...
    # Copy gate into sigmoid scratch: scratch = 0; scratch += gate.
    ...
    output_vram_addr = prog._compiler.get_vram_addr(up_vram.name)
    comparison_params = {
    ...
    print(f"activation output VRAM row: {output_vram_addr // mlen}")
    if args.no_run:
        return {"build_dir": str(build_dir), "ran": False}

    metrics = run_and_assert(build_dir, "gpt_oss_moe_activation", mlen=mlen, blen=blen)
...
def main() -> None:
    ...
    parser.add_argument(
        "--build-dir",
        type=Path,
        default=Path(__file__).parent / "build" / "gpt_oss_moe_activation",
    )
    parser.add_argument("--no-run", action="store_true")
    args = parser.parse_args()
    run_activation_smoke(args)
