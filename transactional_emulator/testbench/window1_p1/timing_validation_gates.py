...
from transactional_emulator.testbench.window1_p1.p1_utils import DEFAULT_OUT_ROOT, ensure_python_paths, load_json, write_json
...
def main() -> int:
    ...
    summary = {
        ...
        "cases": [
            {key: value for key, value in row.items() if key != "stage_profile"}
            for row in rows
        ],
        "gates": gates,
    }
