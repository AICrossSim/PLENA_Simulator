"""
Sub Matrix Transpose Test: 测试子矩阵分块加载和转置计算
使用 AutoCompilerHelper 自动处理 HBM 地址、环境搭建和 golden value 计算

测试场景：
1. 256x256 的大矩阵 W 被分成 4x4 个 64x64 的子块
2. 加载 W[row_idx][:] 到 MRAM（一整行子块）
3. 计算 C = A @T SubMatrix(W, row_idx)
   即 C = A @ W[row_idx*mlen:(row_idx+1)*mlen, :].T
4. 验证结果与 golden value 一致

关键特性：
- sub_projection_T: A @ W[row_idx*mlen:(row_idx+1)*mlen, :].T
- 需要加载 W 的第 row_idx 行子块（该行的所有列）
- 使用 M_TMM 指令进行转置矩阵乘法
...
if __name__ == "__main__":
    ...
    out_features = 256  # output features (必须是 64 的倍数)
    mlen = 64  # 子块大小
    blen = 4   # batch 分块大小
    real_data_ratio = (8*8 + 8) / (8 * 8)  # 1.125
    
    # 子块行索引（测试 sub_projection_T: A @ W[row_idx*mlen:(row_idx+1)*mlen, :].T）
    sub_row_idx = 1  # 取 W 的第 1 行子块（即 W[64:128, :]）
    ...
    # 对于 sub_projection_T，我们取 W[row_idx*mlen:(row_idx+1)*mlen, :] 然后转置
    ...
    # 1. 加载 A 到 VRAM
    # 2. 注册 W 为子矩阵（自动分块）
    # 3. 加载 W[row_idx][:] 到 MRAM（一整行子块）
    # 4. 计算 C = A @T SubMatrix(W, row_idx)
    #    即 C = A @ W[row_idx*mlen:(row_idx+1)*mlen, :].T
    code = f"""
    A_batch = Load_Batch(A)
    W_sub = Register_SubMatrix(W)
    Load_SubMatrix_Row(W_sub, {sub_row_idx})
    C = A_batch @T SubMatrix(W_sub, {sub_row_idx})
    Result C
    """
    ...
    # ========================================================================
    print("=" * 60)
    print("Sub Matrix Transpose Test: sub_projection_T")
    print("=" * 60)
    print(f"Input A: shape {A.shape}")
    print(f"Weight W: shape {W.shape}")
    print(f"\n操作: C = A @ W[row_idx*mlen:(row_idx+1)*mlen, :].T")
    print(f"Sub row index: {sub_row_idx}")
    print(f"取 W 的行范围: W[{sub_row_idx*mlen}:{(sub_row_idx+1)*mlen}, :]")
    print(f"W 行子块形状: ({mlen}, {hidden_size})")
    print(f"W 行子块转置后形状: ({hidden_size}, {mlen})")
    ...
    # 编译并自动设置环境
    result = helper.compile_and_setup(code, asm_name="sub_matrix_T_test")
    ...
    print("Golden Result:")
    print("=" * 60)
    print(f"Golden Result (C = A @ W[{sub_row_idx*mlen}:{(sub_row_idx+1)*mlen}, :].T) shape: {golden_result['original_output'].shape}")
    ...
    W_sub_row = W[sub_row_idx*mlen:(sub_row_idx+1)*mlen, :]  # (mlen, hidden_size)
    C_expected = A @ W_sub_row.T  # (batch_size, mlen)
    ...
    # ========================================================================
    print("\n" + "=" * 60)
    print("Sub Matrix Transpose Test Summary")
    print("=" * 60)
    ...
    print(f"✓ Load_SubMatrix_Row W_sub[{sub_row_idx}][:] -> MRAM (加载行子块)")
    print(f"✓ Sub Projection T: C = A_batch @ W[{sub_row_idx*mlen}:{(sub_row_idx+1)*mlen}, :].T")
