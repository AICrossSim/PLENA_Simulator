...
from transactional_emulator.testbench.moe_timing.replay.utils import OUT_ROOT_BASE
from transactional_emulator.testbench.moe_timing.qwen import (
    build_route_traces,
...
from transactional_emulator.testbench.moe_timing.qwen.run_trace_batch import (
    _prior_run_passed,
    _prune_success_artifacts,
)


DEFAULT_OUT_ROOT = OUT_ROOT_BASE / "campaign"
...
def _routing_output(
    out_root: Path, benchmark: str, model_key: str, layers: list[int], args: argparse.Namespace
) -> Path:
...
def selected_trace_paths(args: argparse.Namespace, selection: dict[str, Any]) -> list[Path]:
    ...
    phases = (
        {"decode"}
        if args.write_phases == "decode"
        else {"prefill"}
        if args.write_phases == "prefill"
        else {"prefill", "decode"}
    )
    paths = []
...
def progress_snapshot(args: argparse.Namespace, selection: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "selected_samples": sum(
            len(obj.get("selected_samples", [])) for obj in selection.get("benchmarks", {}).values()
        ),
...
def _result_ok(build_dir: Path, require_stage_profile: bool = True) -> bool:
    ...
    if require_stage_profile and not (build_dir / "stage_profile.json").exists():
        return False
    # The results file carries `zero_input_smoke_gate`, never a top-level
    # `functional_gate`. Share the batch runner's fail-closed predicate so the two
    # --skip-existing checks cannot drift.
    return _prior_run_passed(result_path)
...
def _worker_run_trace(task: dict[str, Any]) -> dict[str, Any]:
    ...
    cwd = Path(task["cwd"])
    start = time.time()
    # Minimal row first: loading the trace must be inside the try so a corrupt or
    # unreadable trace becomes a failed-status row instead of an exception that
    # unwinds the whole as_completed loop and loses the manifest.
    row: dict[str, Any] = {
    ...
    try:
        ...
        trace_id = trace["trace_id"]
        build_dir = out_root / "trace_replay" / trace_id
        row["trace_id"] = trace_id
        row["build_dir"] = str(build_dir)
...
def _determinism_worker(task: dict[str, Any]) -> dict[str, Any]:
    ...
    cwd = Path(task["cwd"])
    start = time.time()
    # Load the trace inside the try (see _worker_run_trace): a bad trace must not
    # unwind the determinism gate's as_completed loop.
    row: dict[str, Any] = {
        "trace": str(trace_path),
        "worker_pid": os.getpid(),
    }
...
def _normalized_json(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _normalized_json(child) for key, child in sorted(value.items()) if key not in _VOLATILE_JSON_KEYS}
...
def run_determinism_gate(
    args: argparse.Namespace, trace_paths: list[Path], width_info: dict[str, Any] | None
) -> dict[str, Any]:
    ...
    if args.stage_profile:
        # stage_profile.json is only emitted when --stage-profile is set; comparing it
        # otherwise leaves both hashes None -> matches=False -> the gate is a permanent
        # false-negative even when serial and parallel outputs are bit-identical.
        compare_files.insert(1, {"file": "stage_profile.json", "mode": "json"})
...
def run_parallel_replay(
    args: argparse.Namespace, trace_paths: list[Path], width_info: dict[str, Any]
) -> dict[str, Any]:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if "replay" in args.actions:
        ...
        args.replay_run_name == "p3_rev_parallel_replay"
        and (args.out_root / "p3_rev_parallel_replay_manifest.json").exists()
    ):
    ...
    progress = progress_snapshot(args, selection)
    report = write_report(
        args, selection, trace_paths, rss, width, replay_manifest, export_payload, determinism, progress
    )
