...
def numerical_gate(actual, expected, atol, rtol):
    ...
    for token, (row, reference) in enumerate(zip(actual, expected)):
        ...
        for column, (value, target) in enumerate(zip(row, reference)):
            ...
            require(error <= atol + rtol * abs(target),
                    "golden mismatch at token {}, column {}: {} != {}".format(
                        token, column, value, target))
...
def validate_output_pair(bits, values):
    ...
    for bit_row, value_row in zip(bits, values):
        ...
        for bit, value in zip(bit_row, value_row):
            require(isinstance(bit, int) and not isinstance(bit, bool) and 0 <= bit <= 65535,
                    "output BF16 must be uint16")
...
def validate_output_shape(values, tokens, width):
    require(isinstance(values, list) and len(values) == tokens,
            "output token count differs from workload")
    require(all(isinstance(row, list) and len(row) == width for row in values),
...
def validate_run(envelope, golden, workload, architecture, atol, rtol, hbm_channels=8):
    ...
    for field in ("total_ps", "multipliers", "useful_macs", "issued_macs", "hbm_read_bytes",
                  "hbm_write_bytes", "global_dma_inflight_peak", "global_dma_staging_peak_bytes",
                  "combine_sram_peak_bytes", "shared_vector_busy_ps"):
        value = result[field]
    ...
    require(result["global_dma_inflight_peak"] <= architecture["global_dma_credits"],
            "global DMA credits exceeded")
    require(result["global_dma_staging_peak_bytes"] <= architecture["global_dma_staging_bytes"],
            "DMA staging capacity exceeded")
    require(result["combine_sram_peak_bytes"] <= architecture["combine_sram_bytes"],
            "combine SRAM capacity exceeded")
    ...
    for observed, core in zip(result["cores"], architecture["cores"]):
        ...
        for metric in ("useful_macs", "issued_macs", "jobs", "hbm_read_bytes", "compute_busy_ps",
                       "vector_sram_peak_bytes", "accumulator_peak_bytes", "weight_sram_peak_bytes",
                       "weight_slots_peak"):
            require(isinstance(observed[metric], int) and not isinstance(observed[metric], bool)
                    and observed[metric] >= 0, "invalid per-core metric: " + metric)
        ...
        for peak, capacity in (("vector_sram_peak_bytes", "vector_sram_bytes"),
    ...
    require(len(identities) == len(set(identities)) and set(identities) == set(expected_rows),
    ...
    for field in ("useful_macs", "issued_macs", "hbm_read_bytes"):
        require(sum(core[field] for core in result["cores"]) == result[field],
                "per-core totals do not reconcile: " + field)
    ...
    for job in jobs:
        ...
        require(type(job["rows"]) is int
                and job["rows"] == expected_rows[(job["expert"], job["shared"])],
                "expert completion row count differs from workload")
        jobs_per_core[job["core"]] += 1
        rows_per_core[job["core"]] += job["rows"]
        require(all(type(job[field]) is int for field in
                    ("start_ps", "compute_done_ps", "output_copied_ps")),
                "job completion timestamps must be integers")
        require(0 <= job["start_ps"] <= job["compute_done_ps"] <= job["output_copied_ps"]
                <= result["total_ps"], "invalid job completion timestamps")
...
def _run_comparison(binary, workload_path, golden_path, architecture_paths, output_dir,
    ...
    require(all(type(x) in (int, float) and math.isfinite(x) and x >= 0 for x in (atol, rtol)),
            "invalid tolerance")
    ...
    def execute_architecture(index):
        ...
        for repeat in range(repeats):
            ...
            command = [str(binary), "--workload", str(workload_path), "--architecture",
                       str(Path(path).resolve()), "--output", str(report_path),
                       "--hbm-channels", str(hbm_channels)]
    ...
    summary = {
        "schema_version": 1, "all_gates_passed": True,
        "status": "passed", "run_directory": str(run_dir),
        ...
        "repeats": repeats, "hbm_channels": hbm_channels,
        "numerical_tolerance": {"atol": atol, "rtol": rtol}, "comparisons": results,
    }
...
def run_comparison(binary, workload_path, golden_path, architecture_paths, output_dir,
    ...
    summary_path.write_text(json.dumps({"schema_version": 1, "status": "running",
...
def main():
    ...
    for row in summary["comparisons"]:
        print("{}: {} ps, {:.4f}x, gates passed".format(
