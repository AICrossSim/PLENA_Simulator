...
from moe_timing.replay.utils import (
...
def _emulator_stage_names() -> list[str]:
    ...
    Read out of the Rust source rather than mirrored: a mirrored list agrees with
    the original right up until one of them moves.
    """
...
def test_router_is_billed_to_the_routed_branch() -> None:
    ...
    It does not scale with `top_k`, which makes it look routing-independent, but
    it scales with `num_experts` and has no counterpart in a dense or shared-only
    layer.
    """
