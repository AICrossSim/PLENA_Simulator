...
def create_trace(args: argparse.Namespace) -> dict[str, Any]:
    ...
    trace = {
        ...
        "created_by": "transactional_emulator.testbench.moe_timing.replay.create_sample_trace",
...
def main() -> int:
    ...
    parser.add_argument(
        "--reference-path",
        type=Path,
        default=DEFAULT_OUT_ROOT.parent
        / "real_workload"
        / "gpt_oss_layer0_tok1"
        / "reference"
        / "hf_layer0_moe_reference.pt",
    )
    parser.add_argument(
        "--l1-golden-path",
        type=Path,
        default=DEFAULT_OUT_ROOT.parent / "real_workload" / "gpt_oss_layer0_tok1" / "emulator" / "golden_output.pt",
    )
    parser.add_argument(
        "--fixed-route-build-dir", type=Path, default=DEFAULT_OUT_ROOT / "gpt_oss_layer0_tok1_fixed_route"
    )
    parser.add_argument(
        "--device-selected-build-dir", type=Path, default=DEFAULT_OUT_ROOT / "gpt_oss_layer0_tok1_device_selected"
    )
    parser.add_argument(
        "--out", type=Path, default=DEFAULT_OUT_ROOT / "route_traces" / "gpt_oss_layer0_tok1_trace.json"
    )
