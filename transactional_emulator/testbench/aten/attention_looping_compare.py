"""
Compare ATen MHA attention lowering with looped versus Python-unrolled helpers.

This is a codegen/static-analysis harness. It keeps the surrounding GEMM
lowering in the default looped mode for both rows, then toggles only the
attention helper emission through ``prog.unroll_attention``.
"""
...
def forced_aten_unroll(enabled: bool):
    """Force generic ATen GEMM loop mode while emitting this harness."""
...
def emit_aten_mha_one_head(
    *,
    ...
    """Emit the ATen per-head MHA primitive path used by native CLM."""
...
def assert_attention_looping_invariants(rows: list[dict[str, Any]]) -> None:
    ...
    if looped["source_lines"] >= unrolled["source_lines"]:
        failures.append(
            f"looped source lines {looped['source_lines']} >= unrolled {unrolled['source_lines']}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).parent / "build" / "attention_looping_compare",
