...
def build_llada_asm(n_layers: int = 32, n_steps: int = 64,
                    seq_len: int = 64, hidden: int = 64, inter_dim: int = 128, vocab: int = 128) -> Path:
    """
    ...
    print(f"Decoder ASM: {body_start} header lines, "
          f"{len(decoder_lines) - body_start} body lines")
    ...
    result = subprocess.run(
        [sys.executable, str(Path(__file__).parent / "llada_lm_head_asm_gen.py"),
         "--seq-len", str(seq_len),
         "--hidden", str(hidden),
         "--vocab", str(vocab),
         "--build-dir", str(build_dir)],
    ...
    print(f"LM head ASM: {lm_head_body_start} header lines (skipped), "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="LLaDA Multi-Layer Decoder ASM Profile Generator"
    )
    ...
    parser.add_argument("--vocab", type=int, default=128,
                        help="Vocabulary size (sliced)")
    parser.add_argument("--no-profile", action="store_true",
                        help="Skip profiler, just generate ASM")
    ...
    if not args.no_profile:
        print(f"\nRunning ASM profiler on LLaDA ASM "
              f"({args.layers} layers x {args.steps} steps)...")
        root = Path(__file__).parent.parent.parent
        env = os.environ.copy()
        env["PYTHONPATH"] = f"{root}:{root}/tools:{env.get('PYTHONPATH', '')}"
        result = subprocess.run(
            [sys.executable,
             str(root / "analytic_models/roofline/asm_profiler.py"),
             str(asm_path)],
            env=env, capture_output=True, text=True
        )
        print(result.stdout)
