"""
ATen Compiler Test: nn.LayerNorm(64) -> PLENA ISA via compile_module.
...
Note: LayerNorm weight and bias go through HBM but PLENA's layer_norm_plena
does not apply them explicitly — this test validates the normalization pass only.
The golden reference omits weight/bias scaling to match PLENA hardware behavior.
"""
...
if __name__ == "__main__":
    print("=" * 80)
    print("ATen Compiler Test: nn.LayerNorm(128) - Matched Dims")
    print("=" * 80)
    ...
    print(f"Bias:    {model.bias.shape}")

    # ========================================================================
    # CPU golden reference
    # PLENA layer_norm_plena does not apply weight/bias scaling (it uses FPRAM
    # preloaded eps and 1/hidden_size). Golden must match this behavior:
    # compute layer norm without weight/bias.
    # ========================================================================
    ...
    # Build input_tensor dict: weight/bias from state_dict, activation from x
    input_tensor = {}
    ...
    # FP SRAM preload: [0]=0.0, [1]=eps(1e-5), [2]=1/hidden_size
    # layer_norm_plena uses eps_offset=1, reci_hid_offset=2
    ...
    create_mem_for_sim(
        data_size=256,
        mode="behave_sim",
        asm="aten_compiler_layer_norm_matched",
        data=None,
    ...
    # ========================================================================
    run_and_assert(build_dir, "aten_compiler_layer_norm_matched", mlen=mlen, blen=blen)
