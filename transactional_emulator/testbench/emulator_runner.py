...
def run_emulator_repeat_gate(
    ...
    `dump_cwd` is forwarded to every repeat: a caller that isolated its main run
    to a private dump directory must get the same isolation here, or concurrent
    repeat-gate processes would race on the shared default dump directory.
    """
    if repeats < 2:
