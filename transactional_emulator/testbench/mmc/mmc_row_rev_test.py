...
if __name__ == "__main__":
    # Reverse-row prefetch test parameters (B=1, sequence length S)
    seq_len_valid = 27 * 27
    feature_dim = 2048
    vlen = 128
    mlen = 128
    ...
    # Golden output: reverse sequence dimension while preserving per-row feature order.
    golden_output = (
        input_mxfp.view(1, seq_len_kernel, feature_dim).flip(dims=[1]).contiguous().view(seq_len_kernel, feature_dim)
    )
    golden_result = {"input_tensor": input_mxfp, "original_output": golden_output}

    gen_assembly_code = "; MMC Row Reverse (H_PREFETCH_R_V) Test Generation\n"
    gen_assembly_code += f"; Shape: (B=1, S={seq_len_kernel}, D={feature_dim}) reverse-sequence prefetch\n"
    ...
    build_path = Path(__file__).parent / "build" / "mmc_row_rev_test"
    create_sim_env(
    ...
    print("================================================")
    print("Finished generating mmc_row_rev_test assembly code")
    print(
    ...
    if results.get("allclose_pass", False):
        print("\n[mmc_row_rev_test PASSED - emulator numerical check passed]")
    else:
        print("\n[mmc_row_rev_test FAILED - emulator numerical check failed]")
        raise SystemExit(1)
