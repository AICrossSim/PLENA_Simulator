...
if __name__ == "__main__":
    # Reverse-column prefetch test parameters (B=1, logical 27x27 sequence grid)
    rows_valid = 27
    cols_valid = 27
    ...
    gen_assembly_code = "; MMC Col Reverse (Columnwise H_PREFETCH_R_V) Test Generation\n"
    gen_assembly_code += (
        f"; Shape: (B=1, rows={rows_kernel}, cols={cols_kernel}, D={feature_dim}) "
        "reverse-columnwise token-order prefetch\n"
    )
    ...
    build_path = Path(__file__).parent / "build" / "mmc_col_rev_test"
    create_sim_env(
    ...
    print("================================================")
    print("Finished generating mmc_col_rev_test assembly code")
    print(
        "Kernel layout: "
    ...
    if results.get("allclose_pass", False):
        print("\n[mmc_col_rev_test PASSED - emulator numerical check passed]")
    else:
        print("\n[mmc_col_rev_test FAILED - emulator numerical check failed]")
        raise SystemExit(1)