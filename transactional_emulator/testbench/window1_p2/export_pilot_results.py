...
from transactional_emulator.testbench.window1_p1.p1_utils import summarize_run
from transactional_emulator.testbench.window1_p2.p2_utils import OUT_ROOT, load_json, write_csv, write_json
...
def export(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for (benchmark, phase), rows in sorted(groups.items()):
        ...
        taxes = [
            int(tax["routing_tax_cycles"])
            for tax in tax_rows
            for row in rows
            if tax["trace_id"] == row["trace_id"]
        ]
        summary_rows.append(
            {
                "benchmark": benchmark,
