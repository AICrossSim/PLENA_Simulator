...
def parse_buffer_addrs(raw: dict) -> dict:
    """Trivial passthrough — INV_N (= 1/hidden_size) and EPS are now
    inlined as ``T.float16(...)`` literals in the kernel body; the
    compiler auto-hoists them into anonymous ``__const_f16_*``
    global.fpram slots and ``test_helper`` auto-preloads them.
    """
    del raw
    return {}
