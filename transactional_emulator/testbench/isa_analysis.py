...
class SimulatorCycleModel:
    """Opcode cost model matching explicit `cycle!(...)` calls in the simulator."""
    ...
    def description(self) -> str:
        return (
            f"{self.settings_section.lower()} simulator constants from {self.settings_path} "
            f"(DC_EN={self.dc_en}; H_PREFETCH/H_STORE async memory timing not statically charged)"
        )
...
def _latency_value(section: dict[str, Any], name: str, *, dc_en: int) -> int:
    values = section[name]
    return int(values["dc_lib_en" if dc_en else "dc_lib_dis"])


def load_simulator_cycle_model(
    *,
    settings_path: Path | None = None,
    dc_en: int = 1,
) -> SimulatorCycleModel:
    """Load simulator cycle constants from `plena_settings.toml`."""
    ...
    if "BEHAVIOR" in settings:
        settings_section = "BEHAVIOR"
    elif "TRANSACTIONAL" in settings:
        settings_section = "TRANSACTIONAL"
    else:
        raise KeyError("plena_settings.toml must contain either [BEHAVIOR.*] or [TRANSACTIONAL.*]")

    sim_settings = settings[settings_section]
    config = sim_settings["CONFIG"]
    latency = sim_settings["LATENCY"]
    ...
    return SimulatorCycleModel(
        ...
        systolic_processing_overhead=_latency_value(latency, "SYSTOLIC_PROCESSING_OVERHEAD", dc_en=dc_en),
        vector_add_cycles=_latency_value(latency, "VECTOR_ADD_CYCLES", dc_en=dc_en),
        vector_mul_cycles=_latency_value(latency, "VECTOR_MUL_CYCLES", dc_en=dc_en),
        vector_exp_cycles=_latency_value(latency, "VECTOR_EXP_CYCLES", dc_en=dc_en),
        vector_reci_cycles=_latency_value(latency, "VECTOR_RECI_CYCLES", dc_en=dc_en),
        vector_max_cycles=_latency_value(latency, "VECTOR_MAX_CYCLES", dc_en=dc_en),
        vector_sum_cycles=_latency_value(latency, "VECTOR_SUM_CYCLES", dc_en=dc_en),
        scalar_fp_basic_cycles=_latency_value(latency, "SCALAR_FP_BASIC_CYCLES", dc_en=dc_en),
        scalar_fp_exp_cycles=_latency_value(latency, "SCALAR_FP_EXP_CYCLES", dc_en=dc_en),
        scalar_fp_sqrt_cycles=_latency_value(latency, "SCALAR_FP_SQRT_CYCLES", dc_en=dc_en),
        scalar_fp_reci_cycles=_latency_value(latency, "SCALAR_FP_RECI_CYCLES", dc_en=dc_en),
        scalar_int_basic_cycles=_latency_value(latency, "SCALAR_INT_BASIC_CYCLES", dc_en=dc_en),
    )


# Backward-compatible name for older local comparison scripts.
load_behavior_cycle_model = load_simulator_cycle_model
...
def analyze_asm(
    ...
    *,
    ...
    if cycle_model is None:
        cycle_model = load_simulator_cycle_model()
    ...
    return {
        ...
        "opcodes_dynamic": dict(sorted(op_counts.items())),
