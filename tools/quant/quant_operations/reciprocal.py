...
def fp_reciprocal(signed_exponent_in: torch.Tensor, signed_mantissa_in: torch.Tensor, config: dict):
    ...
    reciprocal_mantissa[torch.where(integer_mantissa_in == 0)] = (
        1 * 2 ** (out_fix_width + in_fix_width - 1) - 1
    ) / 2 ** (out_fix_width + in_fix_width - 1)
    reciprocal_mantissa[torch.where(integer_mantissa_in != 0)] = (
        1 / integer_mantissa_in[torch.where(integer_mantissa_in != 0)]
    )
...
def test_reciprocal():
    ...
    qdata_in, exp_in, mant_in = _minifloat_ieee_quantize_hardware(
        data_in, config["in_fix_frac_width"] + config["in_exp_width"] + 1, config["in_exp_width"]
    )
    ...
    assert torch.allclose(golden_out, hardware_out), (
        f"golden_out: {golden_out[torch.where(golden_out != hardware_out)]}, hardware_out: {hardware_out[torch.where(hardware_out != golden_out)]}"
    )
