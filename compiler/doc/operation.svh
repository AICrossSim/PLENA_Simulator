...
typedef enum logic [instruction_pkg::OPCODE_WIDTH - 1:0] {
    ...
    // Multi-head matmul (NEW; additive — per-head independent, see
    // plena_docs/PLENA_BMM_MHA_Diagonal_ISA_Design.md). MHMM = non-transposed
    // (PV), MHTMM = matrix per-head transposed (QK^T). 4th asm operand = g.
    MHMM                   = 6'h35,
    MHTMM                  = 6'h36,
    MHMM_WO                = 6'h37
} CUSTOM_ISA_OPCODE;
