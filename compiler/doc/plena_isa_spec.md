...
**Operation:** `fp_reg<rd> = max(max(Vector[gp_reg<rs1>] & gp_reg<rmask>), fp_reg<rd>)`

**Description:**

Find the maximum over the masked elements of `Vector[gp_reg<rs1>]` and update `fp_reg<rd>` if that maximum exceeds the current value. Accumulates the running max across multiple calls.

When `rmask = 0`, the operation reduces over all VLEN elements unconditionally.  When `rmask = 1`, the per-HLEN-segment head mask in the `V_MASK` register (set by `C_SET_V_MASK_REG`) gates which segments participate; segments with their mask bit cleared do not contribute to the running max.

Use case: causal / sliding-window softmax, where the per-row max must exclude the invalid tail before the `S = exp(S - max)` normalization.
