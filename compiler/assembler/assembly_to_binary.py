...
class AssemblyToBinary:
    ...
    def _convert_to_binary(self, instruction):
        ...
        # 3-operand rmask-style instructions (V_EXP_V / V_RECI_V / V_RED_MAX
        # in ISA spec: ``OPCODE rd, rs1, rmask``) — the parser routes the
        # third operand (numeric literal 0/1) into ``imm`` because it's not a
        # register-prefixed token, leaving ``rmask`` as None.  Normalize here
        # so the rmask-aware encoder branch below sees the correct value at
        # rs3 (bit 18).
        if instruction.opcode in ("V_EXP_V", "V_RECI_V", "V_RED_MAX"):
            if rmask is None and imm is not None:
                ...
                rmask = 0
            # rs2 isn't meaningful for these 3-operand forms; the rmask
            # encoder shifts rs2 into bits 14-17 (unused for these ops).
            if rs2 is None:
                rs2 = 0
        ...
        if instruction.opcode in [
            ...
            "S_ST_INT",
            "S_MAP_V_FP",
            # V_RED_MAX / V_RECI_V / V_EXP_V intentionally NOT here — they
            # need the rmask field (per upstream PLENA_Doc/doc/isa_spec.md:
            # V_RED_MAX rd, rs1, rmask; V_EXP_V rd, rs1, rmask;
            # V_RECI_V rd, rs1, rmask).  The previous placement routed
            # rmask into the imm slot (bit 14), but the decoder reads rmask
            # from rs3 (bit 18) — see op.rs:317 — so mask was silently
            # always 0.  Fix: let them fall through to the rmask-aware
            # encoder branch below.
        ]:
            ...
            # V_EXP_V removed from this 2-operand branch — it has rmask per
            # ISA spec and must use the rmask-aware encoder below.
            # V_RED_SUM kept here: ISA spec is `V_RED_SUM rd, rs1` (no rmask).
