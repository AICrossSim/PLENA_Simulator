...
class AssemblyToBinary:
    ...
    def _convert_to_binary(self, instruction):
        ...
        """
        # Asm alias: MHTMM_WO drains the same 3-D hm_accum as M_BMM_WO.
        if instruction.opcode == "MHTMM_WO":
            instruction.opcode = "M_BMM_WO"
        ...
        # LOCAL PATCH (NOT IN UPSTREAM — see ../../../README.md
        # § "Local patches on top of upstream"): normalize the 3-operand
        # rmask-style instructions ``OPCODE rd, rs1, rmask`` (V_EXP_V /
        # V_RECI_V / V_RED_MAX per ISA spec).  The parser routes the third
        # operand (numeric literal 0/1) into ``imm`` because it has no
        # register prefix; we move it to ``rmask`` so the rmask-aware
        # encoder branch below places it at rs3 (bit 18) where the decoder
        # reads it.
        if instruction.opcode in ("V_EXP_V", "V_RECI_V", "V_RED_MAX"):
            if rmask is None and imm is not None:
                rmask = imm
                imm = None
            elif rmask is None:
                rmask = 0
        ...
        if instruction.opcode in [
            ...
            "S_ST_INT",
            "S_MAP_V_FP",
            # LOCAL PATCH — V_RED_MAX / V_RECI_V / V_EXP_V removed from this
            # imm-encoding list.  Per ISA spec they take ``rmask`` (not imm)
            # at the third operand; the previous placement wrote rmask into
            # bit 14 but the decoder reads it from rs3 @ bit 18.  Letting
            # them fall through to the rmask-aware branch below lands rmask
            # at the correct bit position.
        ]:
            binary_instruction = (imm << (opw + 2 * ow)) + (rs1 << (opw + ow)) + (rd << opw) + opcode
        elif instruction.opcode in ["S_LUI_INT", "M_MV_WO", "M_BMM_WO", "M_BMV_WO", "MHMM_WO"]:
            # MHMM_WO rd, imm — fan-in drain (mh_accum -> vsram), 2-operand like M_BMM_WO.
            binary_instruction = (imm << (opw + ow)) + (rd << opw) + opcode
        elif instruction.opcode in ["S_MV_FP", "S_RECI_FP", "S_EXP_FP", "S_SQRT_FP", "V_RED_SUM"]:
            # LOCAL PATCH — V_EXP_V removed (it has rmask per ISA, must use
            # the rmask-aware branch).  V_RED_SUM stays here: ISA spec
            # defines it as 2-operand ``V_RED_SUM rd, rs1`` without rmask.
        ...
        elif instruction.opcode in ["MHMM", "MHTMM"]:
            # MHMM rd, rs1, rs2, G — 4 operands; the KV-subchunk is the
            # HLEN-aligned column offset inside the rs2 vec address.
            # Multi-head matmul (NEW; additive — does NOT touch M_BMM / M_BTMM).
            #   MHMM/MHTMM  rd, rs1, rs2, G
            #     rd=head_offset_base gp, rs1=matrix gp, rs2=vector gp,
            #     G=group size (4th operand -> `rstride` -> rs3 field, bits 18-21,
            #     where the Rust decoder reads `g`; see op.rs decode 0x35/0x36).
            binary_instruction = (
                (rstride << (opw + 3 * ow))
                + (rs2 << (opw + 2 * ow))
                + (rs1 << (opw + ow))
